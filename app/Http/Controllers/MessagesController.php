...
class MessagesController extends Controller
{
        ...
        public function createSpecific($id)
    {
        ...
        $users = \App\User::where('name', $id)->get();

        return view('messenger.createSpecific', compact('users'));
    }
    /**
    ...
    public function store()
    {
       ...
       if($user == null){
        ...
        Mail::send('emails.message', ['user' => $user, 'sender' => Auth::user()->name, 'body' => $input['message'], 'subject' => $input['subject']], function ($m) use ($user) {
            $m->to($user->email, $user->name)->from('fishark@unc.edu')->subject('You Have A New Message On FishArk!');
        });
